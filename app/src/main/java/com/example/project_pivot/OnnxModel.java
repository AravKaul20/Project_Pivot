...
public class OnnxModel {
    ...
    public OnnxModel(Context context, String modelFileName) throws OrtException, IOException {
        ...
        try (InputStream inputStream = context.getAssets().open(modelFileName)) {
            ...
            Log.d(TAG, "Successfully loaded model: " + modelFileName);
            
            // Log input/output info
            Log.d(TAG, "Input names: " + ortSession.getInputNames());
            Log.d(TAG, "Output names: " + ortSession.getOutputNames());
    ...
    public void close() {
        if (ortSession != null) {
            try {
                ortSession.close();
                Log.d(TAG, "Closed model: " + modelName);
            } catch (OrtException e) {
                Log.e(TAG, "Error closing model: " + modelName, e);
            }
        }
    }
} 